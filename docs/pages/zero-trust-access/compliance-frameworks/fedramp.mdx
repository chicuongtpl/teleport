...
| [AC-20 Use of External Information Systems]((=fedramp.control_url=)AC-20) | Teleport supports connecting multiple independent clusters using a feature called [Trusted Clusters](../deploy-a-cluster/trustedclusters.mdx). When allowing access from one cluster to another, roles are mapped according to a pre-defined relationship of the scope of access. |
...
You also can follow the [Installation instructions](../../installation/single-machine/single-machine.mdx) for
...
to `fips-v1`.  Then, any CA with an Ed25519 key must undergo a [CA rotation](../management/security/ca-rotation.mdx).  The
...
Using Teleport's FIPS binary should not be considered a wholesale security improvement.
It is designed to meet specific regulatory and compliance requirements (such as
FedRAMP), not to increase security in general-purpose deployments.
