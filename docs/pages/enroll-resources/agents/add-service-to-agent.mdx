...
agent's join token to one that authorizes the new services, then restart the
agent. The agent will detect the additional services authorized by the new token
and obtain a new certificate for them.
...
1. Restart the agent, delete the data directory, and start the agent again:
