...
func TestCleanupOrphanedAppServers(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			app0, err := makeStaticApp("app0", staticLabels)
			require.NoError(t, err)
			...
			require.NoError(t, err)
			orphanServer, err := types.NewAppServerV3FromApp(
				orphanApp, "test", s.hostUUID)
			require.NoError(t, err)
			_, err = s.tlsServer.Auth().UpsertApplicationServer(
				t.Context(), orphanServer)
			require.NoError(t, err)

			// Plant an app server record with a different host ID to
			// verify that cleanup does not delete other hosts' records.
			otherServer, err := types.NewAppServerV3FromApp(
				orphanApp, "test", "other-host-id")
			require.NoError(t, err)
			_, err = s.tlsServer.Auth().UpsertApplicationServer(
				t.Context(), otherServer)
			require.NoError(t, err)
			...
			require.EventuallyWithT(t, func(t *assert.CollectT) {
				servers, err := s.authClient.GetApplicationServers(
					s.closeContext, defaults.Namespace)
				if !assert.NoError(t, err) {
					return
				}
				var names []string
				...
				// app0 may or may not have been heartbeated yet; the
				// orphan for our host must be gone.
				assert.NotContains(t, names, s.hostUUID+"/orphan",
					"orphan for this host should have been cleaned up")
				assert.Contains(t, names, "other-host-id/orphan",
					"orphan for other host should remain")
			}, 10*time.Second, 100*time.Millisecond)
		})
	}
}

// TestCleanupOrphanedAppServers_AuthError verifies that a failure to
// list app servers during cleanup does not prevent startup.
func TestCleanupOrphanedAppServers_AuthError(t *testing.T) {
	t.Parallel()
	...
	// If we got here, Start() succeeded. The cleanup goroutine logged
	// the error (if any) but did not block startup.
	apps := s.appServer.getApps()
	require.Len(t, apps, 1)
	require.Equal(t, "app0", apps[0].GetName())
}
